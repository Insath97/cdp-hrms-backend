...
class CdpConnectService
{
    ...
    // In CdpConnectService.php, modify the fetchEmployeeMetrics method:

public function fetchEmployeeMetrics(string $employeeCode, ?string $periodKey = null): ?array
{
    try {
        // Log the request details
        Log::info('CDP API Request', [
            'url' => "{$this->url}/api/v1/external/employees-summary",
            'params' => [
                'search' => $employeeCode,
                'period_key' => $periodKey,
                'per_page' => 10,
            ],
            'has_api_key' => !empty($this->key),
        ...
        if ($response->successful()) {
            ...
            Log::info('CDP API Payload Structure', [
                'keys' => array_keys($payload),
                'has_data_data' => isset($payload['data']['data']),
            ...
            // Find the exact match for employee_code
            foreach ($data as $item) {
                if (isset($item['employee_code']) && strtolower(trim($item['employee_code'])) === strtolower(trim($employeeCode))) {
                    Log::info('Employee found in CDP response', ['employee' => $item]);
                    return $item;
                }
            }

            Log::warning('Employee code not found in CDP response', [
                'employee_code' => $employeeCode,
                'available_codes' => array_column($data, 'employee_code')
            ]);
        } else {
            Log::error('CDP Connect API returned error status', [
                'status' => $response->status(),
